import {
  ...
  export default function Quizzes() {
    ...
    const handleEditQuizTitle = (quizId: string, currentTitle: string) => {
      setEditedTitles({ ...editedTitles, [quizId]: currentTitle });
      dispatch(editQuiz(quizId));
    };
  
    const handleUpdateQuizTitle = async (quiz: any) => {
      const newTitle = editedTitles[quiz._id] || quiz.title;
      const updatedQuiz = { ...quiz, title: newTitle };
      try {
        const result = await quizzesClient.updateQuiz(updatedQuiz);
        dispatch(updateQuiz(result));
        dispatch(editQuiz(quiz._id)); // toggle off edit mode
      } catch (error) {
                ...
                <div className="flex-grow-1">
                  ...
                  <div className="d-flex align-items-center gap-3">
                    ...
                    >
                      ...
                      <Dropdown.Item as={Link} to={`/Kambaz/courses/${cid}/quizzes/${quiz._id}`}>
                        Edit
                      </Dropdown.Item>
                      <Dropdown.Item onClick={() => handleDeleteQuiz(quiz._id)}>
                        Delete
                      </Dropdown.Item>
                      <Dropdown.Item onClick={() => handleEditQuizTitle(quiz._id, quiz.title)}>
                        Rename
                      </Dropdown.Item>
                      <Dropdown.Item onClick={() => handleUpdateQuizTitle(quiz)}>
                        Save Title
                      </Dropdown.Item>
                    </DropdownButton>
  
                    {quiz.published && (
                      <BsCheckCircleFill className="fs-4 text-success" />
                    )}
                  </div>
                </div>
              </li>
