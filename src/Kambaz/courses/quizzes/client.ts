...
import { Attempt, Question } from './quizcontext';
...
export const fetchQuestionsForQuiz = async (quizId: string): Promise<Question[]> => {
...
export const fetchQuizQuestions = async (quizId: string): Promise<Question[]> => {
    const { data } = await axiosWithCredentials.get(`${QUIZZES_API}/${quizId}/questions`);
    return Array.isArray(data) ? data : data.questions || [];  // Make sure to return an array of questions
};

export const submitQuizAttempt = async (attempt: Attempt) => {
    const { data } = await axiosWithCredentials.post(`${QUIZZES_API}/api/attempts`, attempt);
    return data;
};
...
export const fetchUserAttempts = async (userId: string, quizId: string) => {
    try {
        const response = await axios.get(`${REMOTE_SERVER}/api/quizzes/${quizId}/attempts/${userId}`);
